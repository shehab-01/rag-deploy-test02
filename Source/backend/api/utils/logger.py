...
# logger설정(log.yaml 설정파일로 변경)
logger = logging.getLogger("uvicorn")
# handler = logging.StreamHandler()
# logger_format = "{levelprefix:<8} {message}"
# logger_format = "[{asctime}] {levelprefix:<8} {module}.{funcName}:{lineno} - {message}"
# logging.basicConfig(format=logger_format, style="{", level=logging.INFO)
# for handler in logger.handlers:
#     logger_formatter = uvicorn.logging.ColourizedFormatter(logger_format, style="{", use_colors=True)
